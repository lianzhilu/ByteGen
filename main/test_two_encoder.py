...
from main.train_two_encoder import compute_eval_score
...
from c2nl.translator.translator_two_encoder import Translator
from c2nl.translator.beam import GNMTGlobalScorer
from c2nl.translator.translation_two_encoder import TranslationBuilder
...
def add_test_args(parser):
    ...
    files.add_argument('--model_name', type=str, default='code_cfg4',
                       help='Unique model identifier (.mdl, .txt, .checkpoint)')
    files.add_argument('--data_dir', type=str, default='../data/10w/',
                       help='Directory of training/validation data')
    files.add_argument('--dev_src', nargs='+', type=str, default='test/cfgsbt_test.txt',
                       help='Preprocessed dev source file')
    files.add_argument('--dev_cfg', nargs='+', type=str, default='test/code_test.txt',
    ...
    bsearch.add_argument('--use_commandline', type='bool', default=False)
...
def prepare_batch(batch, model):
    ...
    if model.args.copy_attn:
        assert 'src_map' in batch and 'cfg_map' in batch and 'alignment' in batch
        source_map = make_src_map(batch['src_map'])
        source_map = source_map.cuda(non_blocking=True) if args.cuda \
            else source_map
        cfg_map = make_src_map(batch['cfg_map'])
        cfg_map = cfg_map.cuda(non_blocking=True) if args.cuda \
            else cfg_map
        alignment = None
        blank, fill = collapse_copy_scores(model.tgt_dict, batch['src_vocab'])
        blank2, fill2 = collapse_copy_scores(model.tgt_dict, batch['cfg_vocab'])
    else:
        source_map, cfg_map, alignment = None, None, None
        blank, fill, blank2, fill2 = None, None, None, None

    batch_inputs['src_map'] = source_map
    batch_inputs['cfg_map'] = cfg_map
    batch_inputs['alignment'] = alignment
    batch_inputs['blank'] = blank
    batch_inputs['fill'] = fill
    batch_inputs['blank2'] = blank2
    batch_inputs['fill2'] = fill2
    ...
    cfg_word_rep = batch['cfg_word_rep']
    cfg_char_rep = batch['cfg_char_rep']
    cfg_type_rep = batch['cfg_type_rep']
    cfg_mask_rep = batch['cfg_mask_rep']
    cfg_len = batch['cfg_len']
    if args.cuda:
        cfg_len = batch['cfg_len'].cuda(non_blocking=True)
        if cfg_word_rep is not None:
            cfg_word_rep = cfg_word_rep.cuda(non_blocking=True)
        if cfg_char_rep is not None:
            cfg_char_rep = cfg_char_rep.cuda(non_blocking=True)
        if cfg_type_rep is not None:
            cfg_type_rep = cfg_type_rep.cuda(non_blocking=True)
        if cfg_mask_rep is not None:
            cfg_mask_rep = cfg_mask_rep.cuda(non_blocking=True)
    ...
    batch_inputs['cfg_word_rep'] = cfg_word_rep
    batch_inputs['cfg_char_rep'] = cfg_char_rep
    batch_inputs['cfg_type_rep'] = cfg_type_rep
    batch_inputs['cfg_mask_rep'] = cfg_mask_rep
    batch_inputs['cfg_len'] = cfg_len
...
def validate_official(args, data_loader, model):
    ...
    print('%'*50)
    print(list(hypotheses.values())[0])
    nltk_bleu(list(hypotheses.values()), list(references.values()), 'fff')
