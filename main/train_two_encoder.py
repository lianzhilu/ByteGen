...
import c2nl.inputters.utils_two_encoder as util
from c2nl.inputters import constants

from collections import OrderedDict, Counter
from tqdm import tqdm
from c2nl.inputters.timer import AverageMeter, Timer
import c2nl.inputters.vector_two_encoder as vector
import c2nl.inputters.dataset_two_encoder as data

from main.model_two_encoder import Code2NaturalLanguage
...
def add_train_args(parser):
    ...
    files.add_argument('--model_name', type=str, default='testemb_code_cfgsbt',
                       help='Unique model identifier (.mdl, .txt, .checkpoint)')
    files.add_argument('--data_dir', type=str, default='../data/',
                       help='Directory of training/validation data')
    files.add_argument('--train_src', nargs='+', type=str, default='train_cfg.txt',
                       help='Preprocessed train source file')
    files.add_argument('--train_cfg', nargs='+', type=str, default='train_story.txt',
                       help='Preprocessed train cfg file')
    files.add_argument('--train_src_tag', nargs='+', type=str,
                       help='Preprocessed train source tag file')
    files.add_argument('--train_cfg_tag', nargs='+', type=str,
                       help='Preprocessed train cfge tag file')
    files.add_argument('--train_tgt', nargs='+', type=str, default='train_summ.txt',
                       help='Preprocessed train target file')
    files.add_argument('--dev_src', nargs='+', type=str, default='eval_cfg.txt',
                       help='Preprocessed dev source file')
    files.add_argument('--dev_cfg', nargs='+', type=str, default='eval_story.txt',
                       help='Preprocessed dev cfg file')
    files.add_argument('--dev_src_tag', nargs='+', type=str,
                       help='Preprocessed dev source tag file')
    files.add_argument('--dev_cfg_tag', nargs='+', type=str,
                       help='Preprocessed dev cfg tag file')
    ...
    preprocess.add_argument('--cfg_vocab_size', type=int, default=50000,
                            help='Maximum allowed length for cfg dictionary')
    ...
    # log.add_argument('--use_two_encoder', type='bool', default=False)
...
def set_defaults(args):
    ...
    if args.use_commandline:
        if not args.only_test:
            ...
            args.train_cfg_files = []
            args.train_tgt_files = []
            args.train_src_tag_files = []
            args.train_cfg_tag_files = []
            ...
            if num_dataset > 1:
                ...
                if len(args.train_cfg) == 1:
                    args.train_cfg = args.train_cfg * num_dataset
                ...
                if len(args.train_cfg_tag) == 1:
                    args.train_cfg_tag = args.train_cfg_tag * num_dataset
            ...
            for i in range(num_dataset):
                ...
                train_cfg = os.path.join(data_dir, args.train_cfg[i])
                train_tgt = os.path.join(data_dir, args.train_tgt[i])
                if not os.path.isfile(train_src):
                    raise IOError('No such file: %s' % train_src)
                if not os.path.isfile(train_cfg):
                    raise IOError('No such file: %s' % train_cfg)
                ...
                if args.use_code_type:
                    ...
                    train_src_tag = None
                if args.use_cfg_type:
                    train_cfg_tag = os.path.join(data_dir, args.train_cfg_tag[i])
                    if not os.path.isfile(train_cfg_tag):
                        raise IOError('No such file: %s' % train_cfg_tag)
                else:
                    train_cfg_tag = None

                args.train_src_files.append(train_src)
                args.train_cfg_files.append(train_cfg)
                args.train_tgt_files.append(train_tgt)
                args.train_src_tag_files.append(train_src_tag)
                args.train_cfg_tag_files.append(train_cfg_tag)

        args.dev_src_files = []
        args.dev_cfg_files = []
        args.dev_tgt_files = []
        args.dev_src_tag_files = []
        args.dev_cfg_tag_files = []

        num_dataset = len(args.dataset_name)
        if num_dataset > 1:
            if len(args.dev_src) == 1:
                args.dev_src = args.dev_src * num_dataset
            if len(args.dev_cfg) == 1:
                args.dev_cfg = args.dev_cfg * num_dataset
            ...
            if len(args.dev_cfg_tag) == 1:
                args.dev_cfg_tag = args.dev_cfg_tag * num_dataset

        for i in range(num_dataset):
            dataset_name = args.dataset_name[i]
            data_dir = os.path.join(args.data_dir, dataset_name)
            dev_src = os.path.join(data_dir, args.dev_src[i])
            dev_cfg = os.path.join(data_dir, args.dev_cfg[i])
            ...
            if not os.path.isfile(dev_cfg):
                raise IOError('No such file: %s' % dev_cfg)
            ...
            if args.use_code_type:
                ...
                dev_src_tag = None
            if args.use_cfg_type:
                dev_cfg_tag = os.path.join(data_dir, args.dev_cfg_tag[i])
                if not os.path.isfile(dev_cfg_tag):
                    raise IOError('No such file: %s' % dev_cfg_tag)
            else:
                dev_cfg_tag = None

            args.dev_src_files.append(dev_src)
            args.dev_cfg_files.append(dev_src)
            args.dev_tgt_files.append(dev_tgt)
            args.dev_src_tag_files.append(dev_src_tag)
            args.dev_cfg_tag_files.append(dev_cfg_tag)
        ...
        if args.use_src_word or args.use_tgt_word or args.use_cfg_word:
        ...
        if not args.only_test:
            ...
            for i in range(num_dataset):
                ...
                train_cfg = os.path.join(data_dir, args.train_cfg)
                ...
                if args.use_code_type:
                    ...
                    train_src_tag = None
                if args.use_cfg_type:
                    train_cfg_tag = os.path.join(data_dir, args.train_cfg_tag)
                    if not os.path.isfile(train_cfg_tag):
                        raise IOError('No such file: %s' % train_cfg_tag)
                else:
                    train_cfg_tag = None
        ...
        for i in range(num_dataset):
            ...
            dev_cfg = os.path.join(data_dir, args.dev_cfg)
            ...
            if args.use_code_type:
                ...
                dev_src_tag = None
            if args.use_cfg_type:
                dev_cfg_tag = os.path.join(data_dir, args.dev_cfg_tag)
                if not os.path.isfile(dev_cfg_tag):
                    raise IOError('No such file: %s' % dev_cfg_tag)
            else:
                dev_cfg_tag = None
            ...
            args.dev_src_files.append(dev_src)
            args.dev_cfg_files.append(dev_cfg)
            args.dev_tgt_files.append(dev_tgt)
            args.dev_src_tag_files.append(dev_src_tag)
            args.dev_cfg_tag_files.append(dev_cfg_tag)
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    cfg_dict = util.build_word_and_char_dict(args,
                                             examples=train_exs + dev_exs,
                                             fields=['cfg'],
                                             dict_size=args.cfg_vocab_size,
                                             no_special_token=True)
    ...
    # Initialize model
    model = Code2NaturalLanguage(config.get_model_args(args), src_dict, cfg_dict, tgt_dict)
...
def main(args):
    ...
    if not args.only_test:
        ...
        if args.use_commandline:
            for train_src, train_src_tag, train_cfg, train_cfg_tag, train_tgt, dataset_name in \
                    zip(args.train_src_files, args.train_src_tag_files,
                        args.train_cfg_files, args.train_cfg_tag_files,
                        args.train_tgt_files, args.dataset_name):
                train_files = dict()
                train_files['src'] = train_src
                train_files['src_tag'] = train_src_tag
                train_files['cfg'] = train_cfg
                train_files['cfg_tag'] = train_cfg_tag
            ...
            train_src, train_src_tag, train_cfg, train_cfg_tag, train_tgt, dataset_name = args.train_src_files[0], args.train_src_tag_files[0], args.train_cfg_files[0], args.train_cfg_tag_files[0], args.train_tgt_files[0], args.dataset_name
            # print(train_src, train_src_tag, train_src, train_src_tag, train_tgt, dataset_name)
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tag'] = train_src_tag
            train_files['cfg'] = train_cfg
    ...
    dev_exs = []
    if args.use_commandline:
        for dev_src, dev_src_tag, dev_cfg, dev_cfg_tag, dev_tgt, dataset_name in \
                zip(args.dev_src_files, args.dev_src_tag_files,
                    args.dev_cfg_files, args.dev_cfg_tag_files,
                    args.dev_tgt_files, args.dataset_name):
            dev_files = dict()
            dev_files['src'] = dev_src
            dev_files['src_tag'] = dev_src_tag
            dev_files['cfg'] = dev_cfg
            dev_files['cfg_tag'] = dev_cfg_tag
        ...
        dev_src, dev_src_tag, dev_cfg, dev_cfg_tag, dev_tgt, dataset_name = args.dev_src_files[0], args.dev_src_tag_files[0], args.dev_cfg_files[0], args.dev_cfg_tag_files[0], args.dev_tgt_files[0], args.dataset_name
    ...
    if args.only_test:
        ...
        validate_official(args, dev_loader, model, stats)
