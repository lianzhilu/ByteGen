...
from c2nl.models.transformer2 import Transformer
...
class Code2NaturalLanguage(object):
    ...
    def __init__(self, args, src_dict, cfg_dict, tgt_dict, state_dict=None):
        # Book-keeping.
        self.args = args
        self.src_dict = src_dict
        self.args.src_vocab_size = len(src_dict)
        self.cfg_dict = cfg_dict
        self.args.cfg_vocab_size = len(cfg_dict)
    ...
    def update(self, ex):
        ...
        if self.args.copy_attn:
            print(ex.keys())
            assert 'src_map' in ex and 'alignment' in ex and 'cfg_map' in ex

            source_map = make_src_map(ex['src_map'])
            source_map = source_map.cuda(non_blocking=True) if self.use_cuda \
                else source_map

            cfg_map = make_src_map(ex['cfg_map'])
            cfg_map = cfg_map.cuda(non_blocking=True) if self.use_cuda \
                else cfg_map
        ...
        cfg_word_rep = ex['cfg_word_rep']
        cfg_char_rep = ex['cfg_char_rep']
        cfg_type_rep = ex['cfg_type_rep']
        cfg_mask_rep = ex['cfg_mask_rep']
        cfg_len = ex['cfg_len']
        ...
        if self.use_cuda:
            ...
            cfg_len = cfg_len.cuda(non_blocking=True)
    ...
    def predict(self, ex, replace_unk=False):
        ...
        if replace_unk:
            for i in range(len(predictions)):
                ...
                # print(ex['code_tokens'][i])
                # print(ex['cfg_tokens'][i])
                # try:
                predictions[i] = replace_unknown(predictions[i],
                                                 enc_dec_attn,
                                                 src_raw=ex['code_tokens'][i]+ex['cfg_tokens'][i])
                # except:
                #     predictions[i] = replace_unknown(predictions[i],
                #                                      enc_dec_attn,
                #                                      src_raw=ex['cfg_tokens'][i])
                if self.args.uncase:
    ...
    def load(filename, new_args=None):
        ...
        cfg_dict = saved_params['cfg_dict']
        tgt_dict = saved_params['tgt_dict']
        state_dict = saved_params['state_dict']
        args = saved_params['args']
        if new_args:
            args = override_model_args(args, new_args)
        return Code2NaturalLanguage(args, src_dict, cfg_dict, tgt_dict, state_dict)
    ...
    def load_checkpoint(filename, use_gpu=True):
        ...
        args = saved_params['args']
        model = Code2NaturalLanguage(args, src_dict, cfg_dict, tgt_dict, state_dict)
