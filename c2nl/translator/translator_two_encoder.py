...
class Translator(object):
    ...
    def translate_batch(self, batch_inputs):
        ...
        cfg_word_rep = batch_inputs['cfg_word_rep']
        cfg_char_rep = batch_inputs['cfg_char_rep']
        cfg_type_rep = batch_inputs['cfg_type_rep']
        cfg_mask_rep = batch_inputs['cfg_mask_rep']
        cfg_len = batch_inputs['cfg_len']
        cfg_map = batch_inputs['cfg_map']
        alignment = batch_inputs['alignment']
        blank = batch_inputs['blank']
        fill = batch_inputs['fill']
        blank2 = batch_inputs['blank2']
        fill2 = batch_inputs['fill2']
        ...
        if model_name == 'Transformer':
            ...
            memory_bank2, layer_wise_outputs2 = encoder2(cfg_rep, cfg_len)
            cfg_lens = cfg_len.repeat(beam_size)
            dec_states2 = decoder.init_decoder(cfg_lens, memory_bank2.shape[1])
        else:
        ...
        if cfg_lengths is None:
            cfg_lengths = torch.Tensor(batch_size).type_as(memory_bank2) \
                .long() \
                .fill_(memory_bank2.size(1))

        # (2) Repeat src objects `beam_size` times.
        if model_name == 'Transformer' and model.layer_wise_attn:
            memory_bank = [rvar(lwo.data) for lwo in layer_wise_outputs]
            memory_bank2 = [rvar(lwo.data) for lwo in layer_wise_outputs2]
        else:
        ...
        memory_lengths2 = cfg_lengths.repeat(beam_size)
        if cfg_mask_rep is not None:
            cfg_mask_rep = cfg_mask_rep.repeat(beam_size, 1)
        cfg_map = rvar(cfg_map) if self.copy_attn else None
        attn = {"coverage": None}

        # 编码器输出拼接
        memory_bank = torch.cat([memory_bank, memory_bank2], dim=1)
